...
#dt_output=3600    # timestep for output [seconds]
						...
						# to limit IBI files
t0=dt.datetime(2024,1,1)		#  time start WK
t1=dt.datetime(2024,2,1)		#  time end WK
                                # timestep for output [seconds] take as input
dt_output=3600         # if zero take same temporal resolution as input data
					# tine is counted from t0 on
year=str(t0.year) # for in filename check
...
frcdir='/gpfs/work/ksddata/downloads/copernicusmarine/NWS/'
...
if frocingtype=='gcoast':
	...
	date_prefix='_'#
	suffix='_'	
	wild_elev_files='*2D*.nc' # * varying infprmation	
    # download in one file now
	wild_veloU_files='*3D*.nc'	
	wild_veloV_files='*3D*.nc'		
	wild_temp_files='*3D*.nc'		
	wild_salt_files='*3D*.nc'	
...
lon2d=grid['ssh']['longitude']
lat2d=grid['ssh']['latitude']
...
for count,type_file in enumerate(zip(ssh_files,temp_files,salt_files,u_files,v_files)):
	...
	dsi=xr.open_dataset(ssh_file)
	#ssh_dates=ssh_file_dates[count]+np.arange(len(dsi['ssh']))*np.timedelta64(1,'h')
	#ssh_dates=ssh_file_dates[count]+np.arange(len(dsi['zos']))*np.timedelta64(1,'h')

	#datain=ds['ssh'].interp(time=tt)[name_ssh].load().values
	#datain=dsi['ssh'].values
    

	timein=dsi['time'].values
	if count==0:
		dti=timein[1]-timein[0]
		nstep=int(dtout/dti)
	datain=dsi['zos'][::nstep,:].values    
    # here now is 2h ssh
	t00=count*nt_file
...
if plot_bd:
	...
	for varname in 'ssh','salt','temp':
		...
		if varname=='ssh':
			...
			datain=ds['ssh']['zos'][ti_in_file,:].values	            
		else:
